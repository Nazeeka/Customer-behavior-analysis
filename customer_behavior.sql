...
select gender,sum(purchase_amount) as 
revenue from customer group by gender;
...
round(avg(purchase_amount),2) as average_spend, sum(purchase_amount)as total_revenue from customer 
group by subscription_status order by average_spend,total_revenue desc;
